...
const PaymentHistoryNextPayment: React.FC<PaymentHistoryNextPaymentProps> = ({
  paymentAvailable
}) => {
  ...
  return (
    <>
      {paymentAvailable ? (
        <div data-test-id='next-payment-info-div'>
          <h2 className='govuk-heading-m'>{t('PAYMENT_HISTORY_NEXT_PAYMENT_TITLE')}</h2>
          <p className='govuk-body'>{t('PAYMENT_HISTORY_NEXT_PAYMENT_P1')}</p>
          ...
          <p className='govuk-body'>{t('PAYMENT_HISTORY_NEXT_PAYMENT_P3')}</p>
        </div>
      ) : (
